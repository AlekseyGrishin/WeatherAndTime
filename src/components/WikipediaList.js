...
class WikipediaList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    return (
      <div className="WikiApp">
        ...
        <h2>Поиск по Вики</h2>
        <form action="">
          <input type="text" value={this.state.WikiSearchTerms || ''} onChange={this.changeWikiSearchTerms} placeholder='Найдите что-нибудь о своем городе' />
          <button type='submit' onClick={this.useWikiSearchEngine}>Искать</button>
        </form>
        {wikiSearchResults}
      </div>
    );
